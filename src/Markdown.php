<?php
// Rewrite based on parsedown see github https://github.com/erusev/parsedown
...
class Markdown
{
    ...
    protected function blockFencedCodeContinue($Line, $Block)
    {
        ...
        $Block['element']['element']['text'] .= "\n" . $Line['body'];
        if ($Line['body'] === 'gantt' || $Line['body'] === 'sequenceDiagram' || preg_match('/^graph (?:TB|BT|RL|LR|TD);?$/', $Line['body'])) {
    ...
    protected function getEmojiName($code)
    {
        $content = json_decode(file_get_contents(__DIR__ . "/emoji/full.json"), true);
    ...
    protected function inlineUrl($Excerpt)
    {
        if ($this->urlsLinked !== true or !isset($Excerpt['text'][2]) or $Excerpt['text'][2] !== '/') {
            if (preg_match('/:([^\s:]+):?/', $Excerpt['context'], $matches)) {
                $name = $this->getEmojiName($matches[1]);
                if ($name) {
                    $Inline = array(
                        'extent' => strlen($matches[0]),
                        'element' => array(
